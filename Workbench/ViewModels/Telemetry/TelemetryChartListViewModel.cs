...
{ 
    public class TelemetryChartListViewModel : BindableBase, IDialogAware
    {
        ...
        public TelemetryMonitViewModel watchViewModel;
        public TelemetryMonitViewModel WatchViewModel
        {
            get => watchViewModel;
            set
       ...
       => m != null && string.Equals(m.Header, "未选中", StringComparison.Ordinal);
        ...
        {
            WatchViewModel.WatchTelemetryChartGroups.CollectionChanged -= OnChartsChanged;
        }

        public void OnDialogOpened(IDialogParameters parameters)
        {
            WatchViewModel = parameters.GetValue<TelemetryMonitViewModel>("viewModel");
            foreach (var c in WatchViewModel.WatchTelemetryChartGroups)
                if (c.Id == "placeholder")
                    c.Order = int.MinValue;
            WatchGroups = WatchViewModel.WatchTelemetryChartGroups;

            // ★ 列表页用独立视图（不会影响默认视图）
            WatchGroupsView = new ListCollectionView(WatchViewModel.WatchTelemetryChartGroups);
